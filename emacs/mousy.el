...
(defun mousy-scroll-up ()
  (interactive)
  (debug-mousy-points
   (cond ((close-enough current-line top-line) (scroll-up))
	 ((<= current-line (1+ middle-line)) (recenter 0))
	 ((recenter)))))

(defun mousy-scroll-down ()
  (interactive)
  (debug-mousy-points
   (cond ((close-enough current-line bottom-line) (scroll-down))
	 ((>= current-line middle-line) (recenter -1))
	 ((recenter)))))

(defconst mousy-mode-map (make-sparse-keymap))
(define-key mousy-mode-map (kbd "<wheel-down>") 'mousy-scroll-up)
(define-key mousy-mode-map (kbd "<wheel-up>") 'mousy-scroll-down)
